...
with st.expander("View benefit plan codes and descriptions", expanded=False):
    # Prepare legend data with actual costs
    legend_data = []
    
    for code, details in sorted(benefits_lookup.items()):
        is_formula = details.get("is_formula", False)
        
        # For formula benefits, show "Varies by salary"
        # For fixed benefits, show actual costs from Benefits tab
        if is_formula:
            t_cost = "Varies by salary"
            e_cost = "Varies by salary"
            f_cost = "Varies by salary"
        else:
            # Show actual costs from the lookup
...
# Don't format as strings - keep as numbers so sorting works
# Use column_config to format display instead
st.dataframe(
    detail_df, 
    use_container_width=True, 
    hide_index=True,
    column_config={
        # Format all cost columns as currency
