...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[operator-status] DialCallStatus=${dialCallStatus}, action=${request.nextUrl.searchParams.get("action")}`);
    ...
    return twimlResponse(
      [
        say("Thank you for calling. Goodbye.", v, lang),
        `  <Hangup />`,
      ].join("\n")
    );
  } catch (err) {
    console.error("[operator-status] Error:", err);
