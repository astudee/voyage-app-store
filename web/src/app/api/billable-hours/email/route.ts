...
interface EmailRequest {
  ...
  activeEmployees: StaffSummary[];
  contractors: StaffSummary[];
  inactive: StaffSummary[];
  excelBase64: string;
  filename: string;
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { to, startDate, endDate, metricType, summary, activeEmployees, contractors, inactive, excelBase64, filename } = body;
    ...
    const formatStartDate = new Date(startDate).toLocaleDateString("en-US", { month: "long", year: "numeric" });
    const formatEndDate = new Date(endDate).toLocaleDateString("en-US", { month: "long", year: "numeric" });

    const isHours = metricType === "hours";
    const valueLabel = isHours ? "Hours" : "Revenue";

    const renderStaffTable = (staff: StaffSummary[], title: string) => {
      ...
      return `
        ...
        <table>
          <tr>
            <th>Staff Member</th>
            <th class="text-right">Total ${valueLabel}</th>
          </tr>
          ${staff.map((s) => `
            <tr>
              <td>${s.staffName}</td>
              <td class="text-right">${isHours ? fmt(s.totalHours) : fmtCurrency(s.totalRevenue)}</td>
            </tr>
          `).join("")}
        </table>
      `;
...
<body>
  ...
  <p>Period: ${formatStartDate} - ${formatEndDate}</p>
  <p>Generated: ${new Date().toLocaleString()}</p>

  <div style="margin: 20px 0;">
    <div class="summary-box">
      <div class="summary-label">Total ${valueLabel}</div>
      <div class="summary-value">${isHours ? fmt(summary.totalHours) : fmtCurrency(summary.totalRevenue)}</div>
    </div>
    <div class="summary-box">
      <div class="summary-label">Active Employees</div>
      <div class="summary-value">${summary.activeEmployeeCount}</div>
    </div>
    <div class="summary-box">
      <div class="summary-label">Contractors</div>
      <div class="summary-value">${summary.contractorCount}</div>
    </div>
    <div class="summary-box">
  ...
  ${renderStaffTable(activeEmployees, "Active Employees")}
  ${renderStaffTable(contractors, "Contractors")}
  ${renderStaffTable(inactive, "Inactive")}

  <div class="footer">
    <p>Voyage Advisory - Billable ${valueLabel} Report</p>
    <p><em>Detailed monthly breakdown attached as Excel file.</em></p>
  </div>
</body>
</html>`;
    ...
    const mimeMessage = [
      `From: ${delegatedUser}`,
      `To: ${to}`,
      `Subject: Billable ${valueLabel} Report - ${formatStartDate} to ${formatEndDate}`,
      "MIME-Version: 1.0",
